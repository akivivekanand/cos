...
- `today`: log_date, schedule (jsonb array of {time,title,dur,kind: deep|meeting|advising}), hours (jsonb {deep,meetings,advising,open}, open = 8 minus the rest). Written at checkout for the NEXT working day from whatever calendar she pastes; skip schedule if she pastes none.
- `signals`: created_at, lane (paste|drop|flow), filename, content (raw), summary, processed_at. Checkout processes every row with processed_at null, then sets processed_at, writes summary, and nulls content per the transcript rule.
